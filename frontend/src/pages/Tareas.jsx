...
const TareasVistaDocente = () => {
    ...
    const [gradeModal, setGradeModal] = useState({ isOpen: false, entregaId: null, notaActual: '' });
    ...
    // Manejador para abrir modal de calificar
    const handleCalificar = (entregaId, notaActual) => {
        setGradeModal({ isOpen: true, entregaId, notaActual });
    };

    const submitCalificacion = async (nuevaNota) => {
        const calificacionNum = Number(nuevaNota);
        if (isNaN(calificacionNum) || calificacionNum < 0 || calificacionNum > 100) {
            return toast.warning("Por favor ingresa un número válido entre 0 y 100");
        }

        try {
            await calificarEntrega(gradeModal.entregaId, {
            ...
            setEntregas(prev => prev.map(e =>
                e._id === gradeModal.entregaId ? { ...e, calificacion: calificacionNum, estado: 'Calificado' } : e
            ));
            toast.success("Calificación guardada");
            setGradeModal({ isOpen: false, entregaId: null, notaActual: '' }); // Close modal
        } catch (error) {
            console.error(error);
                                ...
                                <tbody>
                                    {entregas.map(entrega => (
                                        <tr key={entrega._id}>
                                            ...
                                            <td>
                                                <span className={`badge-pill ${entrega.estado === 'Calificado' ? 'success' :
...
const TareasVistaEstudiante = ({ user }) => {
    ...
    const handleEntregarTarea = async (e) => {
        e.preventDefault();
        if (!archivoSubir) return toast.warning("Debes seleccionar un archivo para entregar.");
